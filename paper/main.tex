...
% ===========================================================================
\begin{abstract}
Neural retrieval models can exhibit score sensitivity when identity markers are swapped in documents, yet existing audits report only aggregate score differences without explaining where these shifts originate. We leverage ColBERT's decomposable late-interaction scoring to perform a token-level audit of score sensitivity in late-interaction dense retrieval. We introduce \textbf{Token Contribution Disparity (TCD)}, a metric that exactly decomposes counterfactual retrieval-score shifts down to individual query-token contributions. Across \textbf{55{,}440 controlled tests}, supplemented by naturalistic-template and MS~MARCO validations, we find that: (1)~identity-induced score shifts disproportionately concentrate in function words rather than content words, with function words absorbing 1.43$\times$ to 2.08$\times$ more score change; (2)~this pattern is robust across templates, query syntax, and real-world web text; (3)~gender mismatch is the strongest predictor of absolute score sensitivity (1.26$\times$ higher than same-gender pairs), whereas cross-race pairing alone does not independently increase absolute sensitivity; and (4)~subword fragmentation from tokenization yields a modest but consistent amplification. We show that while these score sensitivities are absent in BM25, we observe non-zero aggregate score sensitivity across several neural retrieval architectures, while exact token-level decomposition remains specific to ColBERT. Finally, synthetic candidate-pool ranking simulations demonstrate that these score shifts can produce rank displacements and top-$k$ membership changes.
\end{abstract}
...
ColBERT~\citep{khattab2020} and ColBERTv2~\citep{santhanam2022} preserve per-token representations and compute scores via MaxSim. This decomposability makes them uniquely suited for token-level attribution---a property that single-vector models like DPR~\citep{karpukhin2020} and Contriever~\citep{goldfarb2024} lack. We leverage this property both as the subject and the instrument of our score-sensitivity audit.
...
Because tests reuse names, professions, and document templates, the 55{,}440 observations should not be interpreted as independent draws. We therefore emphasise effect sizes, ratios, mixed-effects robustness, and cluster-aware checks over raw $p$-values. Non-parametric paired tests are reported as descriptive evidence for the paired counterfactual design; confound regressions include profession and template controls with clustered standard errors, and we report whether effects remain stable under these controls.
...
\begin{itemize}
    ...
    \item \textbf{29/33 professions} (88\%) exhibit the pattern (Figure~\ref{fig:professions})
\end{itemize}
...
The four exceptions---janitor ($r$=0.84), electrician ($r$=0.87), plumber ($r$=0.91), and social worker ($r$=0.99)---are cases where the content word itself appears more sensitive to the identity swap, reducing or reversing the function-word dominance.
...
\caption{Func/Cont TCD ratio for each of 33 professions. Coral bars indicate Func~$>$~Cont; purple bars indicate the four exceptions. 29 out of 33 professions show higher function-word TCD.}
...
\begin{itemize}
    \item CC: SS = 0.028; RR: SS = 0.031
    \item \textbf{RR/CC ratio = 1.11$\times$} (Cohen's $d$ = 0.13)
\end{itemize}

The amplification occurs in the \emph{absolute rarity} condition---when both names undergo subword fragmentation---rather than in the token \emph{gap} between names. Since minority-group names are disproportionately fragmented by WordPiece~\citep{petrov2023}, this suggests a potential infrastructure-level fairness concern associated with tokenization fragmentation (Figure~\ref{fig:rarity}).
...
\caption{\textbf{Partition analysis for gender- and race-associated pairings.} Bars show mean Score Sensitivity; error bars show 95\% bootstrap CIs. Cross-gender pairs show higher Score Sensitivity than same-gender pairs, whereas cross-race pairs do not exceed within-race pairs.}
\label{fig:confound}
\end{figure}
...
\paragraph{Name masking (sanity check).} Replacing all name tokens in the \emph{documents} with \texttt{[MASK]} makes the two documents in each counterfactual pair identical, and TCD drops to exactly zero. While this outcome is expected by construction, it verifies that the implementation attributes score changes to the name-bearing counterfactual difference rather than to unrelated bookkeeping or positional artifacts.

\paragraph{MaxSim argmax alignment.} To test whether function-word TCD is simply caused by query tokens switching to different best-matching document tokens, we trace the MaxSim argmax target for each query token across 11{,}088 counterfactual document pairs (24 name pairs $\times$ 33 professions $\times$ 14 templates; 70{,}560 query-token observations; Appendix~\ref{app:robustness}). Function words still show higher mean TCD than content words (1.35$\times$), but they do \emph{not} switch argmax targets more often: their argmax-change rate is slightly lower than content words (0.59 vs.\ 0.61). Nor is the effect explained by direct matching to the name span or its local context: function words hit the name $\pm$2-token window less often than content words (0.35 vs.\ 0.43). This negative result, visualised in Appendix Figure~\ref{fig:argmax_mechanism}, rules out a simple ``function words jump to the name token'' explanation and instead is consistent with contextual representational shifts before the final MaxSim operation, but does not establish a complete causal pathway.

\paragraph{Attention rollout.} We apply attention rollout~\citep{abnar2020} to test whether name tokens route more attention to function-word positions than to content-word positions. The result is \emph{negative}: the attention ratio is 1.00 ($p = 0.065$). Layer-by-layer embedding probing suggests a more distributed representational pathway: at intermediate layers (L1--L9), content-word embeddings shift more than function words (ratio 0.41--0.52$\times$), but at the final layer (L12) function words converge to a 1.07$\times$ ratio. We therefore treat the mechanism as partially characterised rather than fully established.

\paragraph{Query ablation probe.} To test whether function words merely appear as post-hoc attribution carriers or also affect aggregate score sensitivity, we compare the original function-heavy query (``Who is an experienced \{profession\}?'') with a content-only ablated query (``experienced \{profession\}'') across a representative subset of 700 tests. The original query yields slightly higher Score Sensitivity (SS) than the ablated query (mean SS 0.0361 vs. 0.0344; ratio = 1.051$\times$; paired Wilcoxon $p = 1.31 \times 10^{-2}$; Cohen's $d = 0.088$). This small but statistically reliable reduction suggests that function-word context contributes to overall sensitivity, although the effect size is modest and the ablation changes query length and syntax as well as function-word content.

\subsection{Ranking Impact}
\label{sec:results:ranking}
Identity-induced score differences translate into ranking consequences. Across 55{,}440 counterfactual tests, \textbf{77.7\%} exhibit a non-trivial score difference ($>$0.01). In synthetic candidate-pool ranking simulations designed to approximate realistic qualification variance, identity swaps change at least one rank position in 36.6\% (95\% CI: [33.7\%, 39.7\%]), 55.0\% ([52.6\%, 57.1\%]), and 75.1\% ([73.7\%, 76.4\%]) of cases for pool sizes of 10, 20, and 50 documents, respectively. Mean absolute displacement grows with pool size (0.58 [0.52, 0.64], 1.21 [1.12, 1.29], and 3.07 [2.92, 3.21] positions), and top-$k$ membership changes are most visible in the Top-1 (8.5\%, 21.0\%, 41.0\%) and Top-3 (9.8\%, 7.8\%, 18.3\%) sets (Table~\ref{tab:ranking_metrics}). Appendix Table~\ref{tab:margin_flips} further shows that flips heavily concentrate in small-margin adjacent pairs; for pool size 50, 83.9\% of adjacent pairs have a score margin of $<0.05$, where observed rank flip rates reach 25.6\%--31.0\%.

\paragraph{Signed directional analysis.}
Signed score gaps are necessary for distinguishing identity sensitivity from group disadvantage. In the expanded name pool, the direction of score gaps is substantially name-pair dependent: cross-race pairing does not increase absolute score sensitivity after controls, even though individual name pairs can show large signed gaps. We therefore use the signed analysis as a diagnostic for deployment-specific exposure risk rather than as evidence for a global group ordering.

\paragraph{Gender direction.}
Male-associated names score higher than female-associated names in 62.3\% of cross-gender tests (mean gap $+$0.045). This directional pattern is consistent with the stronger absolute sensitivity observed for cross-gender pairs, but should be interpreted as a property of this name pool and query set rather than a universal statement about gender.

\subsection{Naturalistic-Template Validation}
\label{sec:results:naturalistic}

To test whether the function-word sensitivity pattern generalizes beyond our 14 short templates, we apply the identical TCD pipeline to \textbf{200 diverse professional passages} generated from 16 structurally distinct templates spanning 14 professions and 13 knowledge domains (see Appendix~\ref{app:naturalistic} for examples). For each passage we perform the same 5-pair $\times$ 8-query counterfactual analysis, yielding \textbf{8{,}000 additional tests}. The function-word pattern strengthens on these longer naturalistic-template passages (Figure~\ref{fig:naturalistic}): the Func/Cont TCD ratio is \textbf{2.08$\times$} (Cohen's $d$ = 0.94), compared to 1.43$\times$ on the controlled templates. All 8/8 tested query professions show a ratio above 1.0. Removing punctuation from the function-word category does not weaken the pattern (F/C = 1.58$\times$ without ``?''), confirming the result is not driven by punctuation tokens.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{figures/fig6_naturalistic_validation.pdf}
\caption{Function-word sensitivity in controlled templates vs.\ naturalistic-template passages. The effect strengthens on longer diverse passages (2.08$\times$ vs.\ 1.43$\times$, $d$=0.94 vs.\ 0.36), suggesting that the controlled setup is not the only source of the pattern.}
...
To verify whether the function-word sensitivity pattern persists in naturally occurring text, we perform an ecological validation using the standard \textbf{MS~MARCO passage ranking collection}~\citep{msmarco}. We run a SpaCy NER scan over the first 10,000 passages in the collection, identifying passages containing person name entities. For the first 100 naturally occurring passages containing a person name, we perform the counterfactual name-swap audit using our pool of 10 names and 5 professional queries, yielding \textbf{5{,}000 counterfactual tests}. 

We find that the function-word sensitivity pattern holds robustly on naturally occurring MS~MARCO text. The mean Func-TCD is \textbf{0.0162}, whereas the mean Cont-TCD is \textbf{0.0098}, yielding a Func/Cont TCD ratio of \textbf{1.65$\times$}. Function-word amplification occurs in \textbf{70.1\%} of all comparisons, and the overall mean Score Sensitivity is \textbf{0.0276}. This result demonstrates that our findings are not an artifact of template-generated documents, but rather represent a robust representational pattern of contextual late-interaction models processing real-world web text.

% ===========================================================================
\section{Discussion}
\label{sec:discussion}

\paragraph{Function-word sensitivity.}
Non-identity perturbation control (\S\ref{sec:results:control}) shows function-word amplification extends to rare nonsense words (1.67$\times$) but not semantically grounded substitutions, consistent with sensitivity to semantically weak or fragmented tokens. Identity names instantiate this sensitivity in a socially consequential setting because name tokens carry demographic associations as well as frequency and tokenization structure. The naturalistic-template validation (2.08$\times$) and template modulation (0.96$\times$--1.84$\times$) support generalizability beyond a single short template.

\paragraph{Attention null and layer probing.}
Attention rollout did not confirm differential pathways ($p = 0.065$), while layer-by-layer probing and argmax alignment suggest that identity signal is redistributed across contextual representations before the final MaxSim computation. This points to a plausible representational pathway, but further head-level analysis is needed before making a stronger mechanistic claim.

\paragraph{Confound structure.}
Gender mismatch drives sensitivity magnitude (1.26$\times$), while cross-race pairing does not independently increase absolute score sensitivity after controls. Signed score gaps remain important for deployment-specific harm assessment, but in our expanded name pool they are strongly name-pair dependent rather than reducible to a single universal group ordering. Subword fragmentation~\citep{petrov2023, ovalle2024} modestly amplifies the effect, suggesting an infrastructure-level issue that content-level debiasing alone may miss.

\paragraph{ColBERT as diagnostic.}
Integrated gradients produces a concordant function/content ratio (IG: 1.56$\times$, Spearman $\rho = 0.49$, $p < 10^{-44}$), providing an approximate post-hoc check that agrees with TCD's exact MaxSim decomposition.
...
We have presented TCD, a token-level score-sensitivity diagnostic exploiting ColBERT's decomposable MaxSim scoring. Across 55{,}440 controlled tests spanning 65 names from four US-associated racial categories, function words absorb 1.43$\times$ more identity-induced score change than content words; this ratio increases to 2.08$\times$ in the naturalistic-template validation. Gender mismatch is the most stable observed predictor of sensitivity magnitude (1.26$\times$), while cross-race pairing does not independently amplify absolute sensitivity after controls. Practical interventions include \textbf{document-side name masking} (which eliminates TCD but may sacrifice legitimate relevance), \textbf{equitable tokenizer design}, and \textbf{TCD monitoring} as a continuous sensitivity diagnostic.
...
\paragraph{Name pool scope.} Our 65-name pool covers four US-centric race-associated name categories (White, Black, Hispanic, Asian) drawn from established audit studies and the Rosenman ethnicity database. Names from other racial groups, geographies, cultural contexts, and non-binary gender identities are not represented. The stratified pairing strategy improves coverage but cannot fully eliminate confounds between race, gender, frequency, orthography, and tokenization.

\paragraph{Single diagnostic model.} While we compare aggregate sensitivity across BM25 and five neural retrieval architectures, the token-level TCD analysis is specific to ColBERT's MaxSim decomposition. We validate TCD against integrated gradients (Spearman $\rho$ = 0.49, concordant F/C ratio), but extending token-level attribution to other architectures remains future work.

\paragraph{Rarity amplification.} The rarity effect (RR/CC = 1.11$\times$, $d = 0.13$) is modest. A larger, more diverse name pool with controlled frequency, orthography, and tokenizer-fragmentation levels would strengthen this finding.

\paragraph{Attention null result.} Our attention rollout analysis did not confirm the hypothesised differential attention pathway ($p = 0.065$). The mechanism by which function words absorb identity information may not be captured by head-averaged rollout; layer-specific or head-specific analysis could reveal more nuanced patterns.
...
Table~\ref{tab:names} summarises the 65-name pool by race and gender. Names are drawn from \citet{bertrand2004} and the Rosenman et al.\ Harvard Dataverse ethnicity database.
